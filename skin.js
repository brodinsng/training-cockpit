...
;(function(){
  ...
  var FIELDS=[
    ...
    {k:'fixed',label:'Fixed weekly sessions (never move)',type:'text',ph:'e.g. Wed/Fri/Sat FIVE45, Sun CCP'},
    {k:'offdays',label:'Days you cannot train',type:'text',ph:'e.g. Monday'},
  ...
  setInterval(render, 1500); setTimeout(render, 800);
  function profileContext(){ var soft=[], hard=[]; var p1=get('gid_profile'); if(p1){ if(p1.sports)soft.push('sports: '+(Array.isArray(p1.sports)?p1.sports.join(', '):p1.sports)); if(p1.goal)soft.push('goal: '+p1.goal); if(p1.age)soft.push('age: '+p1.age); if(p1.sex)soft.push('sex: '+p1.sex); if(p1.ftp)soft.push('FTP: '+p1.ftp+'W'); if(p1.diet)soft.push('diet: '+p1.diet); if(p1.daysPerWeek)hard.push('aim for about '+p1.daysPerWeek+' training days per week'); } var p2=get('gid_profile2'); if(p2){ if(p2.exp)soft.push('experience: '+p2.exp); if(p2.goal)soft.push('target: '+p2.goal); if(p2.limiter)soft.push('weakest discipline to prioritise: '+p2.limiter); if(p2.sleep)soft.push('typical sleep: '+p2.sleep+'h'); if(p2.rhr)soft.push('resting HR: '+p2.rhr); if(p2.hours)hard.push('keep total weekly training within about '+p2.hours+' hours'); if(p2.injuries)hard.push('AVOID aggravating: '+p2.injuries); if(p2.fixed)hard.push('these are FIXED sessions that MUST stay exactly on their listed days and MUST NOT be moved, replaced, or duplicated: '+p2.fixed); if(p2.offdays)hard.push('schedule NO training at all on these days, mark them Rest: '+p2.offdays); } try{ var d=new Date(); var key='app_checkin_'+d.getFullYear()+'-'+('0'+(d.getMonth()+1)).slice(-2)+'-'+('0'+d.getDate()).slice(-2); var ci=JSON.parse(localStorage.getItem(key)||'null'); if(ci) soft.push('today check-in 1-5: legs '+ci.legs+', sleep '+ci.sleep+', soreness '+ci.sore+', drive '+ci.mot); }catch(e){} var out=''; if(hard.length) out+='HARD CONSTRAINTS you MUST obey exactly (do not violate any): '+hard.join('; ')+'. '; if(soft.length) out+='Athlete profile to tailor to: '+soft.join('; ')+'.'; return out.trim(); }
  var of=window.fetch;
