// GET /api/activities  → recent Strava activities, mapped to the cockpit's shape.
// Uses a stored Strava refresh token (env var) to mint a short-lived access token server-side.
...
export async function onRequestGet({ env }) {
  try {
    ...
    const activities = (Array.isArray(raw) ? raw : []).map((a) => ({
      ...
      // Strava start_date_local is wall-clock local time with a trailing Z; drop it so the client reads it as local.
      ...
      summary: {
        ...
        relative_effort: a.suffer_score ?? 0, // Strava "Relative Effort" = suffer_score
